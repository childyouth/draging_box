...
class App{
    constructor(){
        ...
        this.pixelRatio = window.devicePixelRatio > 1 ? 2 : 1; // retina display를 위해 픽셀비 확인
    ...
    resize(){
        ...
        this.canvas.width = this.stageWidth * this.pixelRatio; // retina display를 위해 픽셀비에 맞게 캔버스 크기 조절
    ...
    onDown(e){
        ...
        for(let i = this.items.length - 1; i>= 0;i--){  // 화면에 그려지는 dialog는 역순이기때문에
            const item = this.items[i].down(this.mousePos.clone());
            if(item){
                this.curItem = item;
                const index = this.items.indexOf(item);
                this.items.push(this.items.splice(index, 1)[0]); // 선택된경우 마지막에 오도록
                break;
            }
        }
    }
